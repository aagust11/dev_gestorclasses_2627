...
import {parseUicon} from '../utils/uicons';
import {validBookmarkImage} from '../utils/bookmarkImages';
...
export default function BookmarkBar({state,subject,onChangeState}:Props){
 ...
 const [uiconInput,setUiconInput]=useState('');
 ...
 const begin=(ownerId:string,link:Bookmark)=>{setError('');setImageBusy(false);setUiconInput(link.uicon||'');setDraft({base:state,ownerId,link:{...link}});};
 ...
 <form className="space-y-3" onSubmit={e=>{e.preventDefault();if(imageBusy)return;try{if(onChangeState?.(saveBookmark(draft.base,draft.ownerId,draft.link),draft.base)!==false)setDraft(null);}catch(e){setError((e as Error).message);}}}>
 ...
 <fieldset><legend className="text-sm font-semibold mb-2">Icona</legend><div className="flex flex-wrap gap-2">{bookmarkIcons.map(key=>{const Icon=icons[key];return <button type="button" key={key} title={labels[key]} aria-label={labels[key]} aria-pressed={!draft.link.image&&!draft.link.uicon&&(draft.link.icon||'link')===key} className={'p-2 rounded border '+((draft.link.icon||'link')===key?'bg-blue-100 border-blue-600':'border-slate-200')} onClick={()=>setDraft({...draft,link:{...draft.link,icon:key,image:undefined,uicon:undefined}})}><Icon size={20}/></button>;})}</div></fieldset>
 <fieldset className="space-y-2 border rounded-lg p-3"><legend className="text-sm font-semibold">Flaticon UIcons</legend>
 <a href="https://www.flaticon.com/uicons" target="_blank" rel="noopener noreferrer" className="ds-button">Buscar UIcons ↗</a>
 <label className="ds-field">Classe o fragment de la icona<input type="text" value={uiconInput} onChange={e=>setUiconInput(e.target.value)} placeholder="fi fi-ss-bell"/></label>
 <button type="button" className="ds-button" disabled={imageBusy} onClick={()=>{const value=parseUicon(uiconInput);if(!value){setError('Enganxa una classe com fi fi-ss-bell o el fragment <i class="fi fi-ss-bell"></i>.');return;}setError('');setDraft({...draft,link:{...draft.link,uicon:value,image:undefined}});}}>Aplicar UIcon</button>
 {draft.link.uicon&&<div className="flex items-center gap-2"><Uicon value={draft.link.uicon}/><span className="text-xs">{draft.link.uicon}</span></div>}
 <p className="text-xs text-slate-500">Les UIcons es carreguen d’internet. Per a altres icones de Flaticon, utilitza una imatge.</p>
 </fieldset>
 <BookmarkImagePicker value={draft.link.image} onBusy={setImageBusy} onChange={image=>setDraft(current=>current?{...current,link:{...current.link,image,uicon:undefined}}:current)}/>
 ...
 {error&&<p role="alert" className="text-rose-700">{error}</p>}<button className="ds-button ds-primary" type="submit" disabled={imageBusy}>Desar accés</button>
 </form></section>;
