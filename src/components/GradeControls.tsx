...
export function GradeBadge({qual,description}:{qual?:string;description?:string}) {
  if(qual==='NP'||qual==='Exempt')return <span title={description} className={`grade-badge ${qual==='NP'?'bg-black text-white':'bg-yellow-100 text-yellow-900 border-yellow-300'}`}>{qual}</span>;
  if(qual==='Fet'||qual==='No fet')return <span className={`grade-badge ${qual==='Fet'?'bg-emerald-100 text-emerald-800':'bg-rose-100 text-rose-800'}`}>{qual}</span>;
