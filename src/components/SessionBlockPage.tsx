...
import {blockLogs,combineBlockLogs,effectiveSessionLogs,getDayBlocks,storeBlockLog} from '../utils/sessionBlocks';
...
export default function SessionBlockPage(p:Props){
  const block=getDayBlocks(p.state,p.dateStr).find(b=>b.id===p.scheduleItemId||b.memberIds.includes(p.scheduleItemId));
  if(!block)return <SessionView {...p}/>;
  ...
  return <SessionView {...p} state={projected} scheduleItemId={block.id} displayStart={block.startTime} displayEnd={block.endTime}
...
function Review(p:Props & {block:ReturnType<typeof getDayBlocks>[number];logs:SessionLog[]}){
  ...
  return <section className="space-y-4"><header className="page-heading"><div><h2 className="text-xl font-bold">Unificar el registre de la sessió</h2><p>{p.dateStr} · {p.block.startTime}–{p.block.endTime}</p></div><button className="ds-button" onClick={p.onBackToTimeline}>Tornar</button></header>
    <p className="ds-panel">Aquestes hores consecutives tenen {p.logs.length} registres previs. Es conservaran tots els comentaris en un sol diari. Revisa les diferències abans d’unificar-los. Als resums, una assistència contradictòria es mostra pendent fins que la resolguis.</p>
