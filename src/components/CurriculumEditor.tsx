...
export default function CurriculumEditor({state,target,onChange,onBack}:{state:AppState;target:CurriculumTarget;onChange:(s:AppState)=>void;onBack:()=>void}) {
  ...
  const [shortLabel,setShortLabel]=useState(criterion?.shortLabel||original?.key||'');
  ...
  const save=(e:React.FormEvent)=>{
    ...
    if(isCriterion)onChange({...state,criteria:state.criteria.map(c=>c.id===target.id?{...c,key:key.trim(),description:description.trim(),shortLabel:shortLabel.trim()||key.trim(),rubric}:c)});
  ...
  return <DetailPage title={isCriterion?'Editar criteri d’avaluació':'Editar competència'} subtitle={original.key} onBack={onBack}>
    <form className="ds-panel space-y-4" onSubmit={save}>
      <div className="grid md:grid-cols-2 gap-4"><label className="ds-field">Codi<input required value={key} onChange={e=>setKey(e.target.value)}/></label>{isCriterion&&<label className="ds-field">Text breu identificatiu<input value={shortLabel} onChange={e=>setShortLabel(e.target.value)}/></label>}</div>
