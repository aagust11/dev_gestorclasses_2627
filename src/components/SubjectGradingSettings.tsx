...
export default function SubjectGradingSettings({subject,onChange}:{subject:Subject;onChange:(s:Subject)=>void}) {
  ...
  return <section className="ds-panel space-y-4 md:col-span-2">
    ...
    {subject.evaluationType==='numeric' && <div className="space-y-2"><div className="flex justify-between items-center"><h4 className="font-semibold">Ítems · suma {items.reduce((n,i)=>n+i.weight,0)}%</h4><button className="ds-button" type="button" onClick={()=>onChange({...subject,numericItems:[...items,{id:crypto.randomUUID(),code:`I${items.length+1}`,name:'',weight:0}]})}>Afegir ítem</button></div>{items.map((item,index)=><div key={item.id} className="border rounded-lg p-3 space-y-3"><div className="grid grid-cols-[1fr_2fr_1fr_auto] gap-2"><input aria-label={`Codi ítem ${index+1}`} value={item.code} onChange={e=>onChange({...subject,numericItems:items.map(i=>i.id===item.id?{...i,code:e.target.value}:i)})}/><input aria-label={`Nom ítem ${index+1}`} placeholder="Exàmens, pràctiques…" value={item.name} onChange={e=>onChange({...subject,numericItems:items.map(i=>i.id===item.id?{...i,name:e.target.value}:i)})}/><input aria-label={`Pes ítem ${index+1}`} type="number" min="0" max="100" step="0.1" value={item.weight} onChange={e=>onChange({...subject,numericItems:items.map(i=>i.id===item.id?{...i,weight:Number(e.target.value)}:i)})}/><button type="button" className="ds-button text-rose-700" onClick={()=>{if(window.confirm(`Eliminar l’ítem «${item.code} · ${item.name}»? Les activitats que el feien servir quedaran sense aquest ítem i els càlculs poden canviar.`))onChange({...subject,numericItems:items.filter(i=>i.id!==item.id)});}}>Eliminar</button></div><NumericAspectsEditor aspects={item.aspects||[]} onChange={aspects=>onChange({...subject,numericItems:items.map(i=>i.id===item.id?{...i,aspects}:i)})}/></div>)}<p className="text-sm text-slate-500">Els pesos sumen 100% i s’apliquen al trimestre i al curs. Dins de cada ítem, les activitats es ponderen pel seu pes.</p></div>}
  </section>;
}
