...
import {getDayBlocks,timetableSettings,minutes,clock} from '../utils/sessionBlocks';
...
export default function HorariView({state,onSelectSession}:Props){
  ...
  return <section id="horari-view-root" className="space-y-3"><header className="page-heading flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">Horari de classes</h2><p className="text-sm text-slate-500">{formatCatalanShortDate(columns[0].iso)} – {formatCatalanShortDate(columns[4].iso)}</p></div><div className="flex gap-2 flex-wrap"><button className="ds-button" aria-label="Setmana anterior" onClick={()=>shift(-7)}>←</button><button className="ds-button" onClick={()=>setCurrentDate(new Date())}>Avui</button><button className="ds-button" aria-label="Setmana següent" onClick={()=>shift(7)}>→</button></div></header>
