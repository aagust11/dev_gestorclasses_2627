import StudentNamesEditor from './StudentNamesEditor';
...
import {AppState, CalculationMode, Student} from '../types';
...
export default function StudentsView({state,onChange,selectedId,onSelect,onSession,onDelete}:{state:AppState;onChange:(s:AppState,base?:AppState)=>boolean|void;selectedId:string|null;onSelect:(id:string|null)=>void;onSession:(slot:string,date:string)=>void;onDelete?:DeleteStudentAction}) {
  const [search,setSearch]=useState('');
  const [group,setGroup]=useState('all');
  const {sort,toggle}=useTableSort();
  const students=sortRows(Object.values(state.studentRegistry||Object.fromEntries(state.subjects.flatMap(s=>s.students).map(st=>[st.id,st]))),sort,(st,key)=>key==='groups'?state.subjects.filter(s=>s.students.some(x=>x.id===st.id)).map(s=>s.name).join(' · '):classroomName(st));
...
function StudentPage({state,student,onChange,onBack,onSession,onDelete}:{state:AppState;student:Student;onChange:(s:AppState,base?:AppState)=>boolean|void;onBack:()=>void;onSession:(slot:string,date:string)=>void;onDelete?:DeleteStudentAction}) {
  ...
  const activeSubjects=state.subjects.filter(s=>!s.isGeneral&&s.students.some(st=>st.id===student.id));
  const subjects=studentSubjects(state,student.id).filter(s=>!s.isGeneral);
  ...
  return <DetailPage title={<StudentName state={state} student={student}/>} subtitle={activeSubjects.map(s=>s.name).join(' · ')} onBack={onBack} actions={<><button className="ds-button" disabled={exporting} onClick={()=>exportReport('word')}>Informe Word</button><button className="ds-button" disabled={exporting} onClick={()=>exportReport('pdf')}>Informe PDF</button></>}>
    <div className="space-y-4">
      ...
      <section className="ds-panel space-y-3"><h3 className="font-bold">Notes i comentaris del trimestre / curs</h3><div className="grade-table-wrap"><table className="grade-table"><thead><tr><th>Assignatura</th><th>Període</th>{methods.map(m=><th key={m}>{methodNames[m]}</th>)}<th>NP / avaluades</th><th>Fet / No fet</th><th>Comentari del període</th></tr></thead><tbody>{subjects.filter(s=>!s.isParent).flatMap(subject=>periods.map(p=><tr key={subject.id+p.id}><td className="font-semibold">{subject.name}<span className="block text-xs text-slate-500">Escala /{subject.evaluationType==='numeric'?10:4}</span></td><td>{p.name}</td>{methods.map(m=>{const evaluation=report.evaluations.find(e=>e.subject.id===subject.id&&e.period.id===p.id);return <td key={m}><GradeComparison grade={evaluation?.actual[m]?.finalGrade} automatic={evaluation?.automatic[m]?.finalGrade}/></td>;})}<td>{(()=>{const s=report.evaluations.find(e=>e.subject.id===subject.id&&e.period.id===p.id)?.activitySummary;return s?`${s.np} / ${s.evaluated}`:'—';})()}</td><td>{(()=>{const s=report.evaluations.find(e=>e.subject.id===subject.id&&e.period.id===p.id)?.activitySummary;return s?`${s.done} / ${s.notDone}`:'—';})()}</td><td className="min-w-56 whitespace-pre-wrap text-sm">{state.periodComments?.[subject.id]?.[p.id]?.[student.id]||'—'}</td></tr>))}</tbody></table></div></section>
      <section className="ds-panel space-y-3"><h3 className="font-bold">Seguiment de les activitats</h3>{report.evaluations.filter(e=>e.period.id==='annual'&&e.activities.length).map(e=><div key={e.subject.id}><h4 className="font-semibold">{e.subject.name}</h4><div className="grade-table-wrap"><table className="grade-table"><thead><tr><th>Activitat</th><th>Lliurament</th><th>Resultat</th><th>Comentari</th></tr></thead><tbody>{e.activities.map(a=><tr key={a.code}><td>{a.code} · {a.title}</td><td>{a.date}</td><td>{a.result}</td><td>{a.comment}</td></tr>)}</tbody></table></div></div>)}</section>
