...
function StudentPage({state,student,onChange,onBack,onSession}:{state:AppState;student:{id:string;name:string};onChange:(s:AppState)=>void;onBack:()=>void;onSession:(slot:string,date:string)=>void}) {
  ...
  const report=useMemo(()=>buildStudentReport(state,student.id,psiVisible&&includePsi),[state,student.id,psiVisible,includePsi]);
  ...
  return <DetailPage title={student.name} subtitle={subjects.map(s=>s.name).join(' · ')} onBack={onBack} actions={<><button className="ds-button" disabled={exporting} onClick={()=>exportReport('word')}>Informe Word</button><button className="ds-button" disabled={exporting} onClick={()=>exportReport('pdf')}>Informe PDF</button></>}>
    <div className="space-y-4">
      {exportError&&<p role="alert" className="text-rose-700">{exportError}</p>}
      <section className="ds-panel"><h3 className="font-bold mb-2">Informació de l’alumne</h3><p className="text-sm text-slate-600 mb-2">{subjects.length} grups / assignatures</p><label className="ds-field">Informació complementària<textarea rows={2} placeholder="Informació útil per al seguiment de l’alumne…" value={profile.notes||''} onChange={e=>saveProfile({notes:e.target.value})}/></label></section>
      <section className="ds-panel space-y-3"><div className="flex justify-between items-center gap-3"><h3 className="font-bold">PSI</h3><button className="ds-button" aria-expanded={psiVisible} aria-controls="student-psi" onClick={()=>{setPsiVisible(v=>!v);setIncludePsi(false);}}>{psiVisible?<EyeOff size={16}/>:<Eye size={16}/>} {psiVisible?'Ocultar PSI':'Mostrar PSI'}</button></div>{psiVisible?<><label id="student-psi" className="ds-field">Contingut del PSI · desat automàtic<textarea rows={8} value={profile.psi||''} onChange={e=>saveProfile({psi:e.target.value})} placeholder="Objectius, mesures i suports, adaptacions i seguiment…"/></label><label className="flex gap-2 text-sm"><input type="checkbox" checked={includePsi} onChange={e=>setIncludePsi(e.target.checked)}/>Incloure el PSI a l’informe</label></>:<p className="text-sm text-slate-500">Contingut ocult. Prem «Mostrar PSI» per consultar-lo o editar-lo.</p>}</section>
