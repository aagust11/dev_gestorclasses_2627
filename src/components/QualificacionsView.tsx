...
import { activitySummary,activityGradeLabel,getAspectScore,calculateSubjectMode, filterActivitiesForPeriod, getCompSettings, getActivityScore, getCriterionScore, scoreToCompetencial, exportActivitiesToExcel, exportTermGradesToExcel } from '../utils/gradeCalculations';
...
export default function QualificacionsView({state,onChangeState}:{state:AppState;onChangeState:(s:AppState,base?:AppState)=>void}) {
  ...
  const students=sortRows(subject.students.filter(st=>st.name.toLocaleLowerCase().includes(search.toLocaleLowerCase())),sort,(st,key)=>{
    if(key==='name')return st.name;
    const split=key.indexOf(':'),kind=key.slice(0,split),id=key.slice(split+1);
    const grade=periodGrades(state,subject,periodId,method)[st.id];
    return kind==='final'?grade?.finalGrade?.score:grade?.[{criterion:'criteria',competency:'competencies',item:'items'}[kind]]?.[id]?.score;
  });
  ...
  const exportSummary=()=>{
    ...
    exportTermGradesToExcel(subject,`${periodName} · ${METHOD_NAMES[method]}`,criteria,competencies,enriched,comments,activities);
  };
  const progress=(id:string)=>{const s=activitySummary(activities,id,subject);return <><td className="summary-divider" title="NP / activitats completament avaluades; exclou pendents i exemptes">{s.np} / {s.evaluated}</td><td title="Activitats de seguiment: fet / no fet">{s.done} / {s.notDone}</td></>;};
  const slots=(a:typeof activities[number])=>a.assessmentType==='completion'?[]:[...(a.numericAspects||[]).map(x=>({id:x.id,label:x.label,aspect:true})),...(a.criteriaIds||[]).map(id=>({id,label:a.criteriaCustomLabels?.[id]||criteria.find(c=>c.id===sourceCriterionId(a,id))?.key||id,aspect:false}))];
  ...
  return <section className="space-y-4 qualifications-page">
    ...
    <p className="text-xs text-slate-600">NP / avaluades: només activitats completament avaluades de cada alumne, sense pendents ni exemptes. Fet / No fet mostra el recompte de les activitats de seguiment.</p>
    {view==='activities'?<div className="grade-table-wrap"><table className="grade-table"><thead><tr><th rowSpan={2}><SortButton label="Alumne/a" column="name" sort={sort} onSort={toggle}/></th><th rowSpan={2}>NP / avaluades</th><th rowSpan={2}>Fet / No fet</th>{activities.map(a=><th key={a.id} colSpan={slots(a).length+1} className="text-center"><button className="text-blue-700 font-bold" onClick={()=>setGradingId(a.id)}>{a.code} · {a.title}</button><span className="block text-xs font-normal">{a.endDate}</span></th>)}</tr><tr>{activities.flatMap(a=>[...slots(a).map(slot=><th key={a.id+slot.id}>{slot.label}</th>),<th key={a.id+'total'}>{a.assessmentType==='completion'?'Fet / No fet':`Global /${numeric?10:4}`}</th>])}</tr></thead><tbody>{students.map(st=><tr key={st.id}><td className="student-cell"><StudentName official state={state} student={st}/></td>{progress(st.id)}{activities.flatMap(a=>[...slots(a).map(slot=>{const score=slot.aspect?getAspectScore(a,st.id,slot.id,subject):getCriterionScore(a,st.id,slot.id,subject),status=a.grades?.[st.id]?.status,g=slot.aspect?a.grades?.[st.id]?.aspectGrades?.[slot.id]:a.grades?.[st.id]?.criteriaGrades?.[slot.id],max=slot.aspect?a.numericAspects?.find(x=>x.id===slot.id)?.maxScore:a.criteriaMaxScores?.[slot.id]??g?.maxScore??10;return <td key={a.id+slot.id}><button className="grade-cell-button" onClick={()=>setGradingId(a.id)}>{status?<GradeBadge qual={status==='not_submitted'?'NP':'Exempt'}/>:score===null?'—':<><span>{g?.rawScore!=null?`${fmt(g.rawScore)} / ${max}`:`${fmt(score)} /4`}</span><GradeBadge qual={scoreToCompetencial(score,settings.thresholds)}/></>}</button></td>;}),<td key={a.id+'total'}><button className="grade-cell-button" onClick={()=>setGradingId(a.id)} title={a.grades?.[st.id]?.comment||'Avaluar activitat'}>{(()=>{const label=activityGradeLabel(a,st.id,subject);return ['Fet','No fet','NP','Exempt'].includes(label)?<GradeBadge qual={label}/>:label;})()}</button></td>])}</tr>)}</tbody></table>{!activities.length&&<p className="p-6 text-slate-500">No hi ha activitats amb lliurament dins d’aquest període.</p>}</div>:<>
