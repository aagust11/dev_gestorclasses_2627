...
import { calculateCompetencialTermGrades, calculateNumericTermGrades, filterActivitiesForPeriod, getCompSettings, getActivityScore, getCriterionScore, scoreToCompetencial, exportActivitiesToExcel, exportTermGradesToExcel } from '../utils/gradeCalculations';
...
export default function QualificacionsView({state,onChangeState}:{state:AppState;onChangeState:(s:AppState)=>void}) {
  ...
  const [edit,setEdit]=useState<{studentId:string;column:Column;value:string}|null>(null);
  ...
  const criteria=state.criteria.filter(c=>competencies.some(x=>x.id===c.competencyId)).sort((a,b)=>(a.order??0)-(b.order??0));
  ...
  const calculate=(m:CalculationMode,existing?:Record<string,TermStudentGrades>)=>numeric?calculateNumericTermGrades(subject,activities,existing,m):calculateCompetencialTermGrades(subject,activities,competencies,criteria,existing,m);
  const all=Object.fromEntries(METHODS.map(m=>[m,record(m)?.cleared?{}:calculate(m,record(m)?.students)])) as Record<CalculationMode,Record<string,TermStudentGrades>>;
  const data=all[method];
  const columns:Column[]=numeric?(subject.numericItems||[]).map(i=>({id:i.id,label:`${i.code} · ${i.weight}%`,description:i.name,type:'item'})):[...criteria.map(c=>({id:c.id,label:c.shortLabel||c.key,description:c.description,type:'criterion' as const})),...competencies.map(c=>({id:c.id,label:c.key,description:c.description,type:'competency' as const}))];
  ...
  const exportSummary=()=>{
    ...
    exportTermGradesToExcel(subject,`${periodName} · ${METHOD_NAMES[method]}`,criteria,competencies,enriched);
  };
  ...
  return <section className="space-y-4 qualifications-page">
    ...
    {view==='activities'?<div className="grade-table-wrap"><table className="grade-table"><thead><tr><th rowSpan={2}>Alumne/a</th>{activities.map(a=><th key={a.id} colSpan={(a.criteriaIds?.length||0)+1} className="text-center"><button className="text-blue-700 font-bold" onClick={()=>setGradingId(a.id)} title={`Avaluar ${a.title}`}>{a.code} · {a.title}</button><span className="block text-xs font-normal">{a.endDate}</span></th>)}</tr><tr>{activities.flatMap(a=>[...(a.criteriaIds||[]).map(id=><th key={a.id+id} title={criteria.find(c=>c.id===sourceCriterionId(a,id))?.description}>{a.criteriaCustomLabels?.[id]||criteria.find(c=>c.id===sourceCriterionId(a,id))?.shortLabel||criteria.find(c=>c.id===sourceCriterionId(a,id))?.key||id}</th>),<th key={a.id+'total'}>Global {numeric?'/10':'/4'}</th>])}</tr></thead><tbody>{students.map(st=><tr key={st.id}><td className="student-cell">{st.name}</td>{activities.flatMap(a=>[...(a.criteriaIds||[]).map(id=>{const score=getCriterionScore(a,st.id,id,subject),g=a.grades?.[st.id]?.criteriaGrades?.[id];return <td key={a.id+id}><button className="grade-cell-button" onClick={()=>setGradingId(a.id)}>{score===null?'—':<><span>{g?.rawScore!==undefined?`${fmt(g.rawScore)} / ${a.criteriaMaxScores?.[id]??g.maxScore??10}`:fmt(score)}</span><GradeBadge qual={scoreToCompetencial(score,settings.thresholds)}/></>}</button></td>;}),<td key={a.id+'total'}><button className="grade-cell-button" onClick={()=>setGradingId(a.id)} title={a.grades?.[st.id]?.comment||'Avaluar activitat'}>{(()=>{const score=getActivityScore(a,st.id,subject);return score===null?'—':<><b>{fmt(score*(numeric?2.5:1))}</b><GradeBadge qual={scoreToCompetencial(score,settings.thresholds)}/></>;})()}</button></td>])}</tr>)}</tbody></table>{!activities.length&&<p className="p-6 text-slate-500">No hi ha activitats amb lliurament dins d’aquest període.</p>}</div>:<>
      ...
      <div className="grade-table-wrap"><table className="grade-table"><thead><tr><th>Alumne/a</th>{columns.map(col=><th key={col.id} title={col.description} className={col.type==='competency'?'bg-blue-50':col.type==='final'?'bg-slate-200':''}><span className="block">{col.label}</span><button className="text-xs text-blue-700 underline font-normal mt-1" aria-label={`Recalcular columna ${col.label}`} onClick={()=>{if(window.confirm(`Recalcular ${col.label} per a tot l’alumnat? Se substituiran les notes manuals d’aquesta columna.`))recalcColumn(col);}}>Recalcular columna</button></th>)}{METHODS.map(m=><th key={m} className="bg-indigo-50">{METHOD_NAMES[m]} total</th>)}</tr></thead><tbody>{students.map(st=><tr key={st.id}><td className="student-cell">{st.name}</td>{columns.map(col=>{const c=cell(data[st.id],col),q=c&&'qual'in c&&typeof c.qual==='string'?c.qual:undefined;return <td key={col.id} className={c?.isManual?'bg-amber-50':''}><button className="grade-cell-button" onClick={()=>setEdit({studentId:st.id,column:col,value:c?.score==null?'':String(c.score)})} aria-label={`${st.name}: modificar ${col.label}`}><span>{fmt(c?.score)}{c?.isManual&&<span className="text-amber-700 ml-1">✎</span>}</span>{q&&<GradeBadge qual={q}/>} {col.type==='final'&&data[st.id]?.finalGrade.autoFailed&&<span className="text-xs text-rose-700">{data[st.id].finalGrade.failedCECount} CE suspeses</span>}</button></td>;})}{METHODS.map(m=><td key={m} className="bg-indigo-50/30"><div className="grade-cell-button"><b>{fmt(all[m][st.id]?.finalGrade.score)}</b><GradeBadge qual={all[m][st.id]?.finalGrade.qual}/></div></td>)}</tr>)}</tbody></table>{!students.length&&<p className="p-6 text-slate-500">No hi ha alumnes per mostrar.</p>}</div>
