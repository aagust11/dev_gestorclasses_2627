...
function document(report:StudentReport):ReportDocument{
  return {title:`Informe de l’alumne · ${report.name}`,filename:`Informe_${report.name}`,teacher:report.teacher,intro:[report.subjects.join(' · '),studentInformation(report),`${report.totals.absent} faltes · ${report.totals.late} retards · ${report.totals.pos} comentaris positius · ${report.totals.incident} incidències. Assistència: ${report.attendance.rate===null?'pendent':report.attendance.rate+'%'}.`,report.supportMeasures?`Mesures de suport: ${report.supportMeasures}`:''],sections:reportSections(report)};
}
