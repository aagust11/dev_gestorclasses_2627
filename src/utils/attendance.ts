...
export function studentAttendance(state:AppState,studentId:string,periodId='annual',subjectId='all',today=toIsoDate(new Date())) {
  const summaries=state.subjects.filter(s=>!s.isGeneral&&!s.isParent&&(subjectId==='all'||s.id===subjectId)&&s.students.some(st=>st.id===studentId)).map(s=>subjectAttendance(state,s,periodId,today)[studentId]);
