import {validTestConfig,DEFAULT_TEST} from './testScoring';
...
const aspect=object({id,label:text,weight:number(0),maxScore:number(Number.MIN_VALUE),format:enumeration('numeric','competencial','test'),test:testConfig},['id','label','weight','maxScore','format']);
const criterionGrade=object({testAnswers,criterionId:id,rawScore:nullable(number(0)),maxScore:number(Number.MIN_VALUE),normalizedScore:nullable(number(0,4)),competencialScore:level});
const activityGrade=object({completion:enumeration('done','not_done'),aspectGrades:map(criterionGrade),testAnswers,score:nullable(number(0,10)),competencialScore:level,status:enumeration('not_submitted','exempt'),comment:text,criteriaGrades:map(criterionGrade),numericItemId:id});
...
const subject=object({id,name:text,color:text,isGeneral:bool,isParent:bool,parentId:nullable(id),students:list(student),evaluationType:enumeration('numeric','competencial'),numericItems:list(object({aspects:list(aspect),id,name:text,code:text,weight:number(0,100)},['id','name','code','weight'])),compSettings:object({values:object({NA:number(0,4),AS:number(0,4),AN:number(0,4),AE:number(0,4)}),thresholds:object({AS:number(0,4),AN:number(0,4),AE:number(0,4)}),maxFailedCompetencies:number(0,Infinity,true)}),generalNotes:list(text),generalLinks:list(object({id,label:text,url:text,icon:text,color:text,shared:bool,image:(v,p,e)=>{if(!validBookmarkImage(v))e(p,'Icona PNG incorporada invàlida o massa gran.');},uicon:(v,p,e)=>{if(typeof v!=='string'||!parseUicon(v))e(p,'Classe UIcons invàlida.');}},['id','label','url']))},['id','name','students']);
const activity=object({assessmentType:enumeration('graded','completion'),numericAspects:list(aspect),criteriaTests:map(testConfig),numericTest:testConfig,id,subjectId:id,code:text,title:text,description:text,startDate:optionalDate,endDate:optionalDate,status:enumeration('auto','not_open','open','pending_correction','corrected'),termId:text,weight:number(0),resources:list(object({id,title:text,url:text},['id','title','url'])),criteriaIds:list(id),criteriaReferences:map(id),criteriaRubrics:map(rubric),criteriaWeights:map(number(0)),criteriaMaxScores:map(number(Number.MIN_VALUE)),criteriaGradingType:map(enumeration('numeric','competencial','test')),criteriaCustomLabels:map(text),numericItemId:text,numericGradingType:enumeration('numeric','competencial','test'),grades:map(activityGrade)},['id','subjectId','title']);
...
const session=object({notHeld:bool,notHeldReason:text,blockMemberIds:list(id),startTime:time,endTime:time,id,scheduleItemId:id,subjectId:id,date,comments:text,nextSessionNotes:text,attendance:map(object({status:enumeration('present','absent','late10','lateMore10','pending'),score:nullable(number(0,10)),posComment:text,regularComment:text,incidentComment:text,posComments:list(text),regularComments:list(text),incidentComments:list(text)}))},['id','scheduleItemId','subjectId','date']);
const config=object({timetable:object({startTime:time,endTime:time,slotMinutes:number(5,120,true)},['startTime','endTime','slotMinutes']),teacherProfile:object({fullName:text,email:text},['fullName','email']),startDate:date,endDate:date,holidays:list(object({date,endDate:optionalDate,label:text},['date','label'])),terms:list(term),timeSlots:list(object({id,name:text,startTime:time,endTime:time},['id','name'])),substitutions:list(object({replacedScheduleItemIds:list(id),id,date,timeSlotId:id,type:enumeration('subject','other'),subjectId:id,customReason:text},['id','date','timeSlotId','type'])),autoClassNotifications:bool,autoClassNotificationMinutes:number(0),reminders:list(object({id,title:text,date,time,type:enumeration('class','calendar','todo'),advanceMinutes:number(0),active:bool},['id','title','date','type','advanceMinutes','active']))});
...
export function inspectImport(value:unknown):ImportReport {
  ...
  if(d.config.timetable&&(!d.config.timetable.startTime||!d.config.timetable.endTime||d.config.timetable.endTime<=d.config.timetable.startTime))error('config.timetable','L’hora final ha de ser posterior a la inicial.');
  for(const sub of d.config.substitutions||[]){
    if(sub.replacedScheduleItemIds&&(!sub.replacedScheduleItemIds.length||new Set(sub.replacedScheduleItemIds).size!==sub.replacedScheduleItemIds.length))error('config.substitutions.replacedScheduleItemIds','La substitució necessita una llista de sessions única i no buida.');
  }
  const blockOwners=new Set<string>();
  for(const log of d.sessionLogs||[]){
    if(log.blockMemberIds){
      if(!log.blockMemberIds.length||!log.blockMemberIds.includes(log.scheduleItemId)||new Set(log.blockMemberIds).size!==log.blockMemberIds.length)error('sessionLogs.blockMemberIds','El bloc necessita membres únics i ha d’incloure la seva sessió principal.');
      for(const id of log.blockMemberIds){const key=JSON.stringify([log.date,id]);if(blockOwners.has(key))error('sessionLogs.blockMemberIds','Dos registres reclamen la mateixa hora d’un bloc.');blockOwners.add(key);}
    }
    if(log.startTime&&log.endTime&&log.endTime<=log.startTime)error('sessionLogs','La durada del bloc no és vàlida.');
  }
  ...
  d.subjects.forEach((s:any,i:number)=>{
    ...
    for(const item of s.numericItems||[])unique(item.aspects||[],path+'.numericItems.aspects');
  ...
  (d.activities||[]).forEach((a:any,i:number)=>{
    ...
    unique(a.numericAspects||[],p+'.numericAspects');
    const checkAnswers=(answers:any,config:any,path:string)=>{if(answers&&config&&answers.correct+answers.blank+answers.incorrect>config.questions)error(path,'El nombre de respostes supera el nombre de preguntes.');};
    for(const [sid,g]of Object.entries(a.grades||{}) as any){
      checkAnswers(g.testAnswers,a.numericTest||DEFAULT_TEST,`${p}.grades[${sid}].testAnswers`);
      for(const [cid,cg]of Object.entries(g.criteriaGrades||{}) as any)checkAnswers(cg.testAnswers,a.criteriaTests?.[cid]||DEFAULT_TEST,`${p}.grades[${sid}].criteriaGrades[${cid}].testAnswers`);
      for(const aspect of a.numericAspects||[]){const ag=g.aspectGrades?.[aspect.id];checkAnswers(ag?.testAnswers,aspect.test||DEFAULT_TEST,`${p}.grades[${sid}].aspectGrades[${aspect.id}].testAnswers`);if(ag?.rawScore!=null&&ag.rawScore>aspect.maxScore)error(p+'.grades.aspectGrades','La nota supera el màxim del subítem.');}
    }
  ...
  const slots=new Set((d.config.timeSlots||[]).map((s:any)=>s.id));const schedules=new Set([...d.schedule,...(d.config.substitutions||[])].map((s:any)=>s.id));
