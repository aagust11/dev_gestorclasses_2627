...
const session=object({blockMemberIds:list(id),startTime:time,endTime:time,id,scheduleItemId:id,subjectId:id,date,comments:text,nextSessionNotes:text,attendance:map(object({status:enumeration('present','absent','late10','lateMore10','pending'),score:nullable(number(0,10)),posComment:text,regularComment:text,incidentComment:text,posComments:list(text),regularComments:list(text),incidentComments:list(text)}))},['id','scheduleItemId','subjectId','date']);
