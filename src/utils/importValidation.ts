...
const term=object({id,name:text,startDate:date,endDate:date},['id','name','startDate','endDate']);
...
const config=object({startDate:date,endDate:date,holidays:list(object({date,endDate:optionalDate,label:text},['date','label'])),terms:list(term),timeSlots:list(object({id,name:text,startTime:time,endTime:time},['id','name'])),substitutions:list(object({id,date,timeSlotId:id,type:enumeration('subject','other'),subjectId:id,customReason:text},['id','date','timeSlotId','type'])),autoClassNotifications:bool,autoClassNotificationMinutes:number(0),reminders:list(object({id,title:text,date,time,type:enumeration('class','calendar','todo'),advanceMinutes:number(0),active:bool},['id','title','date','type','advanceMinutes','active']))});
