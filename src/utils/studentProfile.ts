...
export function studentPeriodGrade(state:AppState, subject:Subject, studentId:string, periodId:string, mode:CalculationMode) {
  ...
  if(record?.cleared)return undefined;
  ...
  return (subject.evaluationType==='numeric'?calculateNumericTermGrades(scoped,activities,record?.students,mode):calculateCompetencialTermGrades(scoped,activities,competencies,criteria,record?.students,mode))[studentId];
}
