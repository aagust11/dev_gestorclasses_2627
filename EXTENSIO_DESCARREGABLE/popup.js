...
async function load(){try{
  ...
  for(const session of res.data.current){const p=document.createElement('p');p.textContent=`${session.name} · ${session.startTime}–${session.endTime}`;$('current').append(p);
    if(session.summary){const s=session.summary;p.append(document.createElement('br'),`${s.recorded}/${s.total} registrades · ${s.pending} pendents`);}
  }
...
$('open').onclick=async()=>{try{const res=await chrome.runtime.sendMessage({channel:'aula-ui',open:true});if(!res.ok)throw Error(res.error);window.close();}catch(e){$('status').textContent=e.message;}};
